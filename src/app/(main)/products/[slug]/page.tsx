...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/orders/check/${product.slug}`, { requiresAuth: true } as InternalAxiosRequestConfig)
      .then((res) => {
        setCanBuy(res.data.canCreate);
        if (!res.data.canCreate) {
          setMessage(res.data.message || "You cannot buy this product now.");
        } else {
          setMessage("");
        }
      })
      .catch(() => {
        setCanBuy(true); // در صورت خطا اجازه خرید بده
        setMessage("");
      });
  }, [product]);
  ...
  async function handleBuy() {
    ...
    try {
      const res = await api.post(
        "/orders",
        { product_id: product.id, pay_currency: payCurrency },
        { requiresAuth: true } as InternalAxiosRequestConfig
      );
